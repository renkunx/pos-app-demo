import { X, RotateCcw, Printer, FileSearch, Calendar, Pencil, Search } from 'lucide-react';
...
export function ActionSheet() {
  ...
  // Close on escape key
  useEffect(() => {
    if (!state.isActionSheetOpen) return;
    const handleEsc = (e: KeyboardEvent) => {
      if (e.key === 'Escape') handleClose();
    };
    window.addEventListener('keydown', handleEsc);
    return () => window.removeEventListener('keydown', handleEsc);
  }, [state.isActionSheetOpen, handleClose]);

  if (!state.isActionSheetOpen) return null;

  const renderContent = () => {
    switch (state.actionSheetType) {
      case 'order_options':
...
function OrderOptionsSheet({ onClose }: { onClose: () => void }) {
  const { state, dispatch } = useAppState();
  const order = state.transactions.find((t) => t.id === state.selectedOrderId);
    ...
    <div className="px-4">
      ...
      <div className="flex gap-3">
        {order.type === 'payment' && order.status === 'success' && (
          <button
            onClick={() => {
              dispatch({ type: 'OPEN_ACTION_SHEET', sheetType: 'refund_confirm' });
            }}
        ...
        <button
          onClick={() => {
            onClose();
            dispatch({ type: 'SHOW_TOAST', message: '补打小票成功' });
          }}
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => {
            ...
            dispatch({ type: 'CLOSE_ACTION_SHEET' });
            dispatch({ type: 'SHOW_TOAST', message: '退款申请已提交' });
          }}
      ...
      <div className="space-y-2">
        {options.map((opt) => (
          <button
            ...
            onClick={() => {
              onClose();
              // In real app would navigate to specific flow
            }}
      ...
      <div className="space-y-2">
        {methods.map((m) => (
          <button
            key={m.key}
            onClick={() => {
              dispatch({ type: 'SET_PAYMENT_METHOD', method: m.key });
              onClose();
            }}
            className={`w-full flex items-center gap-3 p-3.5 rounded-xl text-left transition-all ${
              state.selectedPaymentMethod === m.key
          ...
          >
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center shadow-sm ${
              state.selectedPaymentMethod === m.key ? 'bg-[var(--pos-accent-primary)]' : 'bg-white'
            }`}>
              <FileSearch size={18} className={state.selectedPaymentMethod === m.key ? 'text-white' : 'text-[var(--pos-text-secondary)]'} />
            </div>
...
function QuickAmountSettingsSheet({ onClose }: { onClose: () => void }) {
  const { state, dispatch } = useAppState();
  const [values, setValues] = useState(state.quickAmounts.map(String));

  const handleSave = () => {
    const nums = values.map(Number).filter((n) => !isNaN(n) && n > 0);
    if (nums.length === 4) {
      dispatch({ type: 'UPDATE_QUICK_AMOUNTS', amounts: nums });
      dispatch({ type: 'SHOW_TOAST', message: '快捷金额已更新' });
      onClose();
    }
  };
...
function RefundInputSheet({ onClose }: { onClose: () => void }) {
  ...
  const handleSearch = () => {
    ...
    if (order) {
      ...
      dispatch({ type: 'SHOW_TOAST', message: '未找到可退款的订单' });
    }
  };
